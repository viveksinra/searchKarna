...
export default function InsideDialog(compo) {
  ...
  const getDataWithId = async () => {
    ...
    await axios
      .get(`/api/v1/addition/business/addBusiness/oneData/${myId}`)
...
console.log(allData)
  ...
  const updateVisibility = () => {
    ...
    axios
      .post(`/api/v1/addition/business/addBusiness/updateVisibility/${myId}`,fieldData)
       ...
       <Grid item xs={11}>
        ...
        Location On Map
           ...
           <Button variant="outlined" color="secondary"
            target="_blank" 
            href={`https://publicsearchkaro.vercel.app/oneBusiness/${myId}`} startIcon={<WebIcon />}>
        Website Preview
      </Button>
                     </Grid>   
          {  allImage.map((data, i) =>(
